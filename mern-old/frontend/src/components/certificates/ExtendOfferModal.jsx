...
const ExtendOfferModal = ({ isOpen, onClose, offerLetter, onExtend }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (durationMode === 'calculate') {
      submissionData.newDuration = '';
    } else {
      submissionData.newEndDate = '';
    }

    try {
      await onExtend(offerLetter._id, submissionData);
      onClose();
      // Reset form
      setFormData({
        newValidUntil: '',
        ...
        <div className="p-6">
          ...
          <div className="mb-6 p-4 bg-gray-800 rounded-lg">
            ...
            <p className="text-gray-400 text-sm mt-1">
              Current Acceptance Deadline: {new Date(offerLetter.validUntil).toLocaleDateString()}
            </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <OfferDetailsFormFields
              ...
              prefix="new"
              hideFields={['candidateName', 'email', 'position', 'department', 'offerType', 'companyName', 'salary', 'joiningLocation', 'workType', 'reportingManager', 'benefits', 'hrContact']}
            />
            ...
            <div className="flex gap-3 pt-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className={`flex-1 px-4 py-2 bg-lime-400 text-black font-semibold rounded-md hover:bg-lime-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed`}
              >
